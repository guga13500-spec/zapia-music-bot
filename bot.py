...
async def on_message(message):
    ...
    # Obtém o streamer para o canal atual
    def get_streamer():
    ...
    if texto.lower().startswith("t "):
        ...
        if not streamer_atual.ffmpeg_available:
            canal.send_message("Erro: ffmpeg não está instalado no servidor. Peça ao administrador para instalar.")
            return
        if not streamer_atual.yt_dlp_available:
            canal.send_message("Erro: yt-dlp não está instalado. Peça ao administrador para instalar.")
            return

        canal.send_message(f"Procurando: {query}...")
        pausado = False

        # Roda a busca em thread para não travar o bot
    ...
    if texto.lower() == "p":
        s = get_streamer()
        if pausado:
            canal.send_message("Continuando a música...")
            pausado = False
            # yt-dlp/ffmpeg não suporta pausa nativa; recomeçamos o stream
            # (limitação da biblioteca — informar o usuário)
            canal.send_message("(Nota: retomada não suportada nesta versão — use 't <música>' para tocar de novo)")
        else:
    ...
    if texto.lower().startswith("v "):
        try:
            ...
            s = get_streamer()
            s.volume = vol
            canal.send_message(f"Volume ajustado para {vol}%")
        except ValueError:
            canal.send_message("Use: v <número entre 0 e 100>  (ex: v 80)")
        return
